...
import { useAudioFilesData, audioFilesColumns, generateAudioFilesRows, generateAudioFilesSummary } from "../data/audioFilesData";
...
function AudioFiles() {
  const { audioFiles, loading, regionCounts, callStatusCounts } = useAudioFilesData();
  ...
  // Process audio files data when loaded
  useEffect(() => {
    if (!loading) {
      const generatedRows = generateAudioFilesRows(audioFiles);
      setRows(generatedRows);
      setFilteredRows(generatedRows);
      setSummary(generateAudioFilesSummary(regionCounts, callStatusCounts));
    }
  }, [loading, audioFiles, regionCounts, callStatusCounts]);
          ...
          {loading ? (
            ...
            <Card>
              <VuiBox p={2}>
                ...
                <VuiBox mt={2}>
                  <VuiProgress value={summary?.successRate || 0} color="success" sx={{ background: "#2D2E5F" }} label={false} />
                </VuiBox>
              </VuiBox>
            </Card>
          )}
          ...
          <Card>
            <VuiBox p={3}>
              ...
              {loading ? (
                ...
                <VuiBox>
                  {summary?.callStatusBreakdown && Object.entries(summary.callStatusBreakdown)
                    ...
                    .map(([status, count]) => {
                      ...
                      const getStatusColor = (status) => {
                        ...
                        if (["busy", "failed", "no-answer"].includes(status)) return "error";
                          ...
                          <VuiBox display="flex" justifyContent="space-between" alignItems="center" mb={1}>
                            <VuiBox display="flex" alignItems="center">
                              ...
                              <VuiTypography variant="button" color="white" fontWeight="medium">
                                {status === "No Call" ? status : status.charAt(0).toUpperCase() + status.slice(1)}
                              </VuiTypography>
                            </VuiBox>
                            <VuiTypography variant="button" color="white" fontWeight="medium">
                              {count}
